...
test_php_fpm() {
    ...
    # 检查 PHP-FPM 是否在 9000 端口监听
    local port_listen
    port_listen=$(docker_exec sh -c "ss -tlnp 2>/dev/null | grep ':9000' || netstat -tlnp 2>/dev/null | grep ':9000'") || port_listen=""
    if [[ -n "${port_listen}" ]]; then
        pass "PHP-FPM 在端口 9000 正常监听"
    else
        # 尝试使用其他方式检查
